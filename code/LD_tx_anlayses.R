...
sig56_genes=intersect(sig5$zfin_id_symbol,sig6$zfin_id_symbol)
...
sig.pre_LD = deg.cpm7[c(which(deg.cpm7$zfin_id_symbol%in%sig56_genes)),] %>% 
  filter(abs(logFC)>log2(1.5)) %>% filter(FDR <0.01)
nrow(sig.pre_LD) # =1  rgs5b
  #.. overlapping 45 genes show same direction of regulation, but are not part of DEG3 (no significant differences following LD between wt and bPAC+).
  #.. In addition, almost all of them are not DEGs in pre-LD condition except 1 genes (rgs5b) 
  #.. This result indicates that those genes are involved in normal response to LD and not significantly ELS-primed.

sig46_genes=intersect(sig4$zfin_id_symbol,sig6$zfin_id_symbol)
sig6$zfin_id_symbol[c(which(sig6[c(which(sig6$zfin_id_symbol%in%sig46_genes)),"logFC"]*sig4[c(which(sig4$zfin_id_symbol%in%sig46_genes)),"logFC"] <0))]
  #.. DEG1 and DEG3 have 3 overlapping gene names, but oppositely regulated. Thus, they are part of LD_DEGs

ab.LD.res = unique(c(sig5$id,sig4$id,sig6$id))
ab.LD.res=ab.LD.res[- c(which(ab.LD.res %in% sig56$id))]
...
LD.deg.list = LD.deg.list[-c(which(LD.deg.list$zfin_id_symbol %in% sig56$zfin_id_symbol)),]
...
DEG 1                   18   18.002    -0.002       0.000
DEG 2                  469  468.998     0.002       0.000
DEG 3                  931  931.001    -0.001       0.000
DEG 1&DEG 2             45   45.002    -0.002       0.000
DEG 1&DEG 3              2    0.000     2.000       0.001
DEG 2&DEG 3           1116 1116.000     0.000       0.000
DEG 1&DEG 2&DEG 3        0    0.002    -0.002       0.000
...
library(jsonlite)
#up-regulated
LD.deg_all.up=filter(LD.deg.list, logFC > 0)
...
#.. "MTF2"   "FOXM1"  "EGR1"   "MYBL2"  "EN2"    "E2F7"   "CSRNP1",for up-reg LD-dges.  
#.. CSRNP1" "E2F7" are part of LD_DEGs

#top10 TF .but only 9 are detected
tf.bp.data = data.frame("TF"=top.tf.LD[1:9],"rank"=top.tf.LD.rank[1:9],"score"=top.tf.LD.score[1:9])
...
#down-regulated
LD.deg_all.dn=filter(LD.deg.list, logFC < 0)
...
#top10 TF
tf.bp.data = data.frame("TF"=top.tf.LD[1:8],"rank"=top.tf.LD.rank[1:8],"score"=top.tf.LD.score[1:8])
...
##### sig.any int. ld-deg
LD_deg= unique(c(LD.deg_all.dn$zfin_id_symbol, LD.deg_all.dn$zfin_id_symbol))
...
tiff("./figures/s.Fig4D_sb.6to120LD.sp.sn.8x30.tiff", height = 28, width = 8, res = 1200, compression = "lzw", units = "cm")
...
#write(zh.LD_DEG ,"./data/zh.LD_DEG.txt",sep = "\n")
#write(zh.LD_DEG.dn,"./data/zh.LD_DEG.dn.txt",sep = "\n")
#library(reticulate)
#source_python("./code/disignet_enricher.py")
#library(org.Hs.eg.db)
#prot_id_map.dis <- clusterProfiler::bitr(unique(zh_LD_list), "SYMBOL","ENTREZID", "org.Hs.eg.db")
...
try(res_enrich <-disease_enrichment( entities =zh_LD_list, vocabulary = "HGNC",
                                 database = "CURATED" ) # "ALL"
,silent =T)
table.dis.LD <- res_enrich@qresult
table.dis.LD.sig = table.dis.LD %>% arrange(desc(gg)) %>% filter(FDR <0.05) %>% filter(Count >= 30)
write.csv(table.dis.LD.sig,"./outputs/s.table6_DisGeNET_enrichment.csv")

tiff("./figures/Fig6A_top.disgenet.tiff", width = 13, height = 12,units = "cm", res = 1200, compression = "lzw")
plot(res_enrich, class = "Enrichment", count =30,  cutoff= 0.01, nchars=70)
dev.off()


try(res_enrich.dn <-disease_enrichment( entities =zh.LD_DEG.dn, vocabulary = "HGNC",
                                 database = "CURATED" )
,silent = T )
table.dis.LD.dn <- res_enrich.dn@qresult
table.dis.LD.dn.sig = table.dis.LD.dn %>% arrange(desc(gg)) %>% filter(FDR <0.05) %>% filter(Count >= 30)
write.csv(table.dis.LD.dn.sig,"./outputs/s.table6_DisGeNET_enrichment.dn.csv")

tiff("./figures/Fig6A_top.disgenet.dn.tiff", width = 13, height = 12,units = "cm", res = 1200, compression = "lzw")
plot(res_enrich.dn, class = "Enrichment", count =30,  cutoff= 0.01, nchars=70)
dev.off()
...
schizo.                                  105    105         0       0.013
Autistic                                  39     39         0       0.005
bipolar                                   35     35         0       0.004
depressive&schizo.                        10      0        10       0.034
depressive&Autistic                        4      4         0       0.000
depressive&bipolar                        10     10         0       0.001
schizo.&Autistic                           8      8         0       0.001
schizo.&bipolar                           29     29         0       0.004
...
tiff(paste0("./figures/Fig6C_dp.heatmap.10x5_",nrow(st_sig_tx_1) ,".tiff"), height = 10,width = 5, res=1200, units = "cm", compression = "lzw")
...
tiff(paste0("./figures/Fig6C_sch.heatmap.10x5_",nrow(st_sig_tx_1),".tiff"), height = 10,width = 5, res=1200, units = "cm", compression = "lzw")
...
tiff(paste0("./figures/Fig6C_auti.heatmap.10x5_",nrow(st_sig_tx_1),".tiff"), height = 10,width = 5, res=1200, units = "cm", compression = "lzw")
...
tiff(paste0("./figures/Fig6C_bipolar.heatmap.10x5_",nrow(st_sig_tx_1),".tiff"), height = 10,width = 5, res=1200, units = "cm", compression = "lzw")
...
tiff(paste0("./figures/s.Fig6E_top.dis.LD.DEGS.heatmap.",nrow(st_sig_tx_1) ,".tiff"), height = 10,width = 5, res=1200, units = "cm", compression = "lzw")
...
LD_DEG_disg.up = LD.all.disig8 %>% filter(logFC >0) %>% filter(FDR <0.01) %>% arrange(desc(abs(logFC)))
LD_DEG_disg.dn = LD.all.disig8 %>% filter(logFC <0) %>% filter(FDR <0.01) %>% arrange(desc(abs(logFC)))

zh.LD_DEG_disg.up = unique(filter(zh.all,zfin_id_symbol %in% LD_DEG_disg.up$zfin_id_symbol)[,2])
zh.LD_DEG_disg.dn = unique(filter(zh.all,zfin_id_symbol %in% LD_DEG_disg.dn$zfin_id_symbol)[,2])

#write(zh.LD_DEG_disg.up[- c(which(zh.LD_DEG_disg.up %in% c("FBXL3","PRODH")))], "./outputs/zh.LD_DEG_disg.up.txt")
#write(zh.LD_DEG_disg.dn[- c(which(zh.LD_DEG_disg.up %in% c("FBXL3","PRODH")))][1:(150+19)], "./outputs/zh.LD_DEG_disg.dn.txt")
...
hc = hclustfunc(distfunc(st_sig_tx_1))
ht.label.order = hc$labels[hc$order]
write(ht.label.order,"./figures/Fig6D_trans_temporal.LD.DEGS.label.order.txt")
nrow(maint.ht.mt)#57
tiff(paste0("./figures/s.Fig6a_trans_temporal.LD.DEGS.heatmap_",nrow(st_sig_tx_1) ,".tiff"), height = 7,width = 5, res=1200, units = "cm", compression = "lzw")
try(heatmap.2((st_sig_tx_1), col=mycol, scale='none',
              dendrogram = "row",Colv = F,breaks=breaks,hclustfun=hclustfunc, distfun=distfunc,
              density='density', trace = 'none',
              na.color = F,
...
tiff(paste0("./figures/s.Fig6b_hidden.ELSprimed.LD.DEGS.heatmap.4x4_",nrow(st_sig_tx_1) ,".tiff"), height = 7,width = 5, res=1200, units = "cm", compression = "lzw")
...
hc = hclustfunc(distfunc(st_sig_tx_1))
ht.label.order = hc$labels[hc$order]
write(ht.label.order,"./figures/Fig6D.ELSprimed.LD.DEGS.label.order.txt")
...
tiff(paste0("./figures/Fig6E_dis.ELSprimed.LD.DEGS.heatmap.",nrow(st_sig_tx_1) ,".tiff"), height = 10,width = 5, res=1200, units = "cm", compression = "lzw")
...
##### version for all LD-DEGs


#selection
for (i in c(5:8)) {
  deg = get(paste0("deg.cpm",i))
  deg = deg %>% filter(zfin_id_symbol %in% unique(LD.deg.list$zfin_id_symbol))
...
##### sig.any int. ld-deg
dep.int.ld = intersect(LD_deg,sig.any)

###s.fig7 # comparison with PNAS, https://www.pnas.org/doi/10.1073/pnas.1820842116, Provençal et al., 2020

ft.LD.deg = deg.cpm8 %>% filter(t.test <0.05) %>% 
                          filter(mean.cpm.pos > 5|mean.cpm.wt >5)

hz_LD_list.up = filter(zh.all,zfin_id_symbol %in% 
                      intersect(ft.LD.deg$zfin_id_symbol[c(which(ft.LD.deg$FDR <0.01))],ft.LD.deg$zfin_id_symbol[c(which(ft.LD.deg$logFC > log2(1.3)))]))
hz_LD_list.dn = filter(zh.all,zfin_id_symbol %in% 
                         intersect(ft.LD.deg$zfin_id_symbol[c(which(ft.LD.deg$FDR <0.01))],ft.LD.deg$zfin_id_symbol[c(which(ft.LD.deg$logFC < -log2(1.3)))]))

#Supplementary table 5: List of significantly regulated Pro-diff+WO +acute transcripts that map to a long-lasing DMS (n=702).
s5_pnas = read.csv("./supplymentary/pnas.1820842116.sd01_st5.csv")

s5_up= s5_pnas %>% filter(FC_pro.diff.WO.acute >0) %>% 
  filter(Gene_symbol%in% hz_LD_list.up$human.Symbol)
s5_dn= s5_pnas %>% filter(FC_pro.diff.WO.acute <0) %>% 
  filter(Gene_symbol%in% hz_LD_list.dn$human.Symbol)
